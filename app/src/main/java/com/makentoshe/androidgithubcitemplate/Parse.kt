...
class Parse {
    ...
    fun parsingSong(args: String): Song {
        ...
        m["Authorization"] = "Bearer rEUF2cIT7X1xdQ1CkDIRIAjreSMKauHI68IlzmpKuduZU5bmwXmxPdY_M2Ybdzub"
        ...
        var artist=Artist(responseSong.getJSONObject("primary_artist").getString("name"),responseSong.getJSONObject("primary_artist").getLong("id"))
    ...
    fun parsingSearch(args: String): ArrayList<Song> {
        ...
        for (i in 0 until responseSearch.length()){
            ...
            var name=responseSearch.getJSONObject(i).getJSONObject("result").getString("title")
            var artist = Artist(responseSearch.getJSONObject(i).getJSONObject("result").getJSONObject("primary_artist").getString("name"), responseSearch.getJSONObject(i).getJSONObject("result").getJSONObject("primary_artist").getLong("id"))
